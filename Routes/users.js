...
const createUserToken = (userId) => {
    return jwt.sign({ id: userId }, config.jwt.pass, config.jwt.expires);
}
...
router.post('/create', async (req, res) => {
    ...
    try {
        ...
        if (!user) return res.status(500).send({ error: 'Erro ao criar usuário!' });
...
router.post('/auth', async (req, res) => {
    ...
    try {
        ...
        user.password = undefined;
        return res.status(200).send({ user, token: createUserToken(user._id) });
    } catch(err) {
        return res.status(500).send({ error: 'Erro ao buscar usuário!' });
    }
});
